...
namespace SmartQuant.Xml
{
	public class XmlDocumentBase : XmlDocument
	{
		...
		private const string SCHEMA_VERSION_ATTRIBUTE = "schemaVersion";

		public int SchemaVersion {
			get {
				if (!this.DocumentElement.HasAttribute (SCHEMA_VERSION_ATTRIBUTE))
					return 1;
				else
					return this.GetInt32Attribute (SCHEMA_VERSION_ATTRIBUTE);
			}
		}
		...
		{
			this.LoadXml (string.Format ("{0}", name));
			this.InsertBefore (this.CreateXmlDeclaration (Vau4EkONyhQVGwxidg.hrqvXeKlE (158), Encoding.Unicode.HeaderName, null), this.DocumentElement);
			if (description != null)
				this.InsertBefore (this.CreateComment (description), this.DocumentElement);
			this.SetAttribute (SCHEMA_VERSION_ATTRIBUTE, schemaVersion);
		}
